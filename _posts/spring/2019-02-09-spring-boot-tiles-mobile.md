...
> **[2026년 추가]** 이 글에서 쓴 **spring-mobile 프로젝트는 이미 End of Life 상태**다 (2022년에 `spring-attic`으로 아카이브됨, 이 글에서 받아 쓴 `2.0.0.M2`가 사실상 마지막으로 나온 버전이고 정식 릴리즈는 끝내 안 나왔다). 지금 새로 이런 기능이 필요하다면 이 라이브러리를 설치하려 하지 말 것 — 대신 아래 두 방향 중 하나를 권장한다.
> 1. **서버사이드 분기 자체를 없애고 반응형(Responsive) 프론트엔드로 대체.** 요즘은 하나의 뷰를 CSS 미디어 쿼리로 데스크탑/모바일에 맞게 다르게 보여주는 게 표준이라, 이 글에서 하는 "URL 앞에 mobile/을 붙여 아예 다른 JSP를 렌더링"하는 방식 자체가 트렌드에서 많이 벗어났다.
> 2. **정말 서버에서 기기 종류를 구분해야 한다면** `User-Agent` 헤더를 직접 파싱하는 가벼운 라이브러리(`ua-parser`류)를 쓰거나, 간단한 정규식으로 직접 판별하는 필터를 만드는 편이 이제 존재하지 않는 프레임워크에 의존하는 것보다 유지보수하기 쉽다.
