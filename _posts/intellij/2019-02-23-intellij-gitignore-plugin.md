...
> **[2026년 추가]** 지금은 이 기능이 **IDE에 기본 내장**되어 있어서 아래 플러그인을 따로 설치할 필요가 없다. 파일/폴더 우클릭 → `Git` → `Add to .gitignore` (또는 `.git/info/exclude`) 로 바로 쓸 수 있다. 아래 플러그인 설치 과정은 예전 버전 기준이니 참고만 하면 된다.
